...
def assign_profile(cluster):
    if cluster == 0:
        return "Gold (Cluster 0): These customers have the highest total spend, total transactions, and total purchased. They make purchases frequently (approximately every 69 days), buy a lot of items (around 12 per transaction), and spend a lot of money (around 13,289,760 units). They buy from a moderate number of product lines (around 6) and have a high RFM score (around 4,993,502)."
    elif cluster == 1:
        return "Standard (Cluster 1): These customers have very low engagement. They make purchases less frequently (approximately every 72 days), buy the fewest items (around 7 per transaction), and spend the least amount of money (around 5,761,472 units). They buy from a moderate number of product lines (around 9) and have the lowest RFM score (around 2,206,356)."
    elif cluster == 2:
        return "Bronze (Cluster 2): These customers have lower values in total spend, total transactions, and total purchased. They make purchases less frequently (approximately every 85 days), buy fewer items (around 10 per transaction), and spend less money (around 10,565,260 units). They buy from a large number of product lines (around 530) and have a moderate RFM score (around 3,975,258)."
    elif cluster == 3:
        return "Silver (Cluster 3): These customers have moderate values in total spend, total transactions, and total purchased. They make purchases less frequently (approximately every 268 days), buy a moderate number of items (around 10 per transaction), and spend a moderate amount of money (around 11,339,110 units). They buy from a moderate number of product lines (around 7) and have a high RFM score (around 5,050,553)."
    else:
        return "Unknown Profile"

# Predict the cluster for the new customer
customer_cluster = pipeline.predict(new_customer)

# Assign profile based on predicted cluster
customer_profile = assign_profile(customer_cluster[0])

# Display the predicted profile
st.write(f"The predicted profile for the new customer is: {customer_profile}")
# Get summary statistics of the selected cluster
selected_cluster_stats = df_clean[df_clean['Clusters'] == customer_cluster[0]].describe()
